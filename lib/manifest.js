  ...
  for (const m of manifest.promoted_methods) {
    ...
    try {
      ch.record({
        ...
        content: m.content,
        domain: 'method',
        // record() scrubs content but stores tags raw (tags are controlled-vocab
        // on every internal write path). A manifest is externally sourced, so its
        // tags are untrusted free text — scrub each here, the same chokepoint
        // discipline recordMethodCandidate applies to its tool-classes. A scrub
        // throw propagates to the per-method try/catch below: the write is dropped
        // and logged as an error, never persisted raw (the inverted fail-open path).
        tags: Array.isArray(m.tags) ? m.tags.map(t => secrets.scrub(String(t))) : [],
        layer: 'ground_truth'
      });
      imported++;
    } catch (e) {
