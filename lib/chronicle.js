...
// `created_at` is optional and defaults to Date.now() — every normal caller omits
// it (session activity is genuinely "now"). It exists for reference knowledge that
// must carry ZERO recency weight in recall(): recall ranks on
// `-(bm25) + recencyBoost` where recencyBoost decays to 0 past recencyWeightDays,
// so a backdated row (e.g. created_at:0) surfaces ONLY on strong FTS token overlap
// and never rides a recency bump onto a generic query. The atlas loader uses this;
// it is NOT exposed via the MCP record tool, so the tool surface is unchanged.
function record({ session_id, content, domain, tags, intensity, layer, created_at }) {
  ...
  const info = stmt.run(
    ...
    layer || 'hypothesis',
    typeof created_at === 'number' ? created_at : Date.now()
  );
...
function logCompass({ session_id, tool_name, action_summary, classification, rule_matched, reason, user_override }) {
  ...
  db().prepare(`
    INSERT INTO compass_log (session_id, tool_name, action_summary, classification, rule_matched, reason, user_override, occurred_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    session_id || null,
    tool_name,
    ...
    rule_matched || null,
    safeReason || null,
    // Controlled audit marker (the consumed approval's row id), never secret-bearing.
    user_override || null,
    Date.now()
  );
}
...
function getCompassSince({ since_id = 0, limit = 50 } = {}) {
  try {
    return db()
      .prepare(
        `SELECT id, session_id, tool_name, action_summary, classification, rule_matched, reason, user_override, occurred_at
...
function getCompassHistory({ limit = 20, classification, matched_only } = {}) {
  ...
  const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';
  const sql = `SELECT id, session_id, tool_name, action_summary, classification, rule_matched, reason, user_override, occurred_at FROM compass_log ${where} ORDER BY occurred_at DESC LIMIT ?`;
  params.push(limit);
