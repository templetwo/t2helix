...
function setGoal({ session_id, goal, why, acceptance_criteria }) {
  ...
  const goalChanged = !!(prior && prior.goal && prior.goal !== goal);
  ...
  // OF THIS GOAL. Explicit criteria always win. Otherwise preserve the prior
  // boundary ONLY for an idempotent re-set (same goal text) — a genuinely new
  // goal starts unbounded so stale criteria can't bleed across and the
  // decomposition offer can fire. (The prior goal + its criteria are already
  // archived above, so nothing is lost.)
  let criteriaJson;
  if (Array.isArray(acceptance_criteria)) {
    criteriaJson = JSON.stringify(acceptance_criteria);
  } else if (!goalChanged && prior && prior.acceptance_criteria) {
