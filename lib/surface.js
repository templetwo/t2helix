...
const { denoiseErrorQuery } = require('./query-normalize');
...
// Imperative task verbs. A prompt that LEADS with one of these is a task, even
// when it's short and carries no path/identifier token ("run tests", "deploy the
// worker", "rebuild") — gating those off silenced exactly the recall a task
// wants (adversarial review, 2026-06-16). Acks of the same shape ("run it",
// "merge it", "do it") are caught by ACK_RE first, so they never reach here.
const TASK_VERBS = new Set([
  'fix', 'run', 'deploy', 'build', 'rebuild', 'test', 'merge', 'add', 'remove',
  'refactor', 'ship', 'debug', 'check', 'update', 'install', 'revert', 'push',
  'pull', 'create', 'delete', 'write', 'implement', 'rename', 'move', 'rollback',
  'migrate', 'patch', 'wire', 'enable', 'disable', 'configure', 'review', 'audit',
  'investigate', 'trace', 'profile', 'optimize', 'verify', 'diagnose', 'restore'
]);
...
function isTrivialPrompt(p) {
  ...
  // A prompt that LEADS with an imperative task verb is a task — keep recall on
  // even when it's short and identifier-less ("run tests", "rebuild").
  const firstWord = (t.toLowerCase().match(/[a-z]+/) || [''])[0];
  if (TASK_VERBS.has(firstWord)) return false;
...
function selectInjection({ goal, prompt, recall }) {
  ...
  if (!trivial) {
    try {
      ...
      // De-noise a pasted traceback so the error identity (not file paths /
      // line numbers) leads the FTS query against the error-fix atlas. No-op on
      // ordinary prompts.
      const query = denoiseErrorQuery(prompt);
      const promptTokens = significantTokens(query);
      const raw = recall({ query, topK }) || [];
      hits = raw
        // Defense in depth: methods surface ONLY via the targeted lookup above.
        // recall() already excludes domain:'method' by default, but drop it here
        // too so a stray include_meta on this path can never push a method into
        // the generic firehose — the cardinal invariant survives a future edit.
        .filter(h => h && h.domain !== 'method')
